//! Shared text preparation and PCM shaping for TTS output.
//!
//! Mental model:
//!
//! ```text
...
/// Character budget for greedy synthesis chunks after the latency-critical
/// first sentence.
pub const TTS_CHUNK_MAX_CHARS: usize = 200;

/// Pocket's output sample rate.
pub const TTS_SAMPLE_RATE: usize = 24_000;
const FADE_OUT_SAMPLES: usize = TTS_SAMPLE_RATE * 8 / 1_000;
const LEAD_IN_SAMPLES: usize = TTS_SAMPLE_RATE * 20 / 1_000;
const TRAILING_SILENCE_SAMPLES: usize = TTS_SAMPLE_RATE * 80 / 1_000;

/// Convert submitted assistant text into desktop-compatible synthesis chunks.
///
/// The first sentence stands alone to minimize time to first playback.
/// Remaining sentences are packed greedily up to 200 characters.
pub fn prepare_tts_chunks(raw: &str) -> Vec<String> {
    let text = preprocess_for_tts(raw);
    if text.is_empty() {
        return Vec::new();
    }
    let sentences: Vec<String> = split_sentences(&text)
        ...
        .collect();
    group_sentences_into_chunks(&sentences, TTS_CHUNK_MAX_CHARS)
}

/// Group already-prepared sentences using Pocket's desktop chunking policy.
pub fn group_sentences_into_chunks(sentences: &[String], max_chars: usize) -> Vec<String> {
...
/// Clamp and shape a Pocket chunk for glitch-free playback.
///
/// Applies the desktop 8 ms fade-out, then prepends 20 ms and appends 80 ms
/// of silence, preserving the established 100 ms inter-chunk gap.
pub fn shape_tts_chunk(samples: Vec<f32>) -> Vec<f32> {
    let mut audio: Vec<f32> = samples
        .into_iter()
        .map(|sample| sample.clamp(-1.0, 1.0))
        .collect();
    let fade = FADE_OUT_SAMPLES.min(audio.len() / 2);
    for index in 0..fade {
        let sample_index = audio.len() - 1 - index;
        audio[sample_index] *= index as f32 / fade as f32;
    }

    let mut shaped = Vec::with_capacity(LEAD_IN_SAMPLES + audio.len() + TRAILING_SILENCE_SAMPLES);
    shaped.resize(LEAD_IN_SAMPLES, 0.0);
    shaped.extend(audio);
    shaped.resize(shaped.len() + TRAILING_SILENCE_SAMPLES, 0.0);
    shaped
}
...
mod tests {
    ...
    #[test]
    fn preparation_keeps_first_sentence_latency_chunk_separate() {
        assert_eq!(
            prepare_tts_chunks("First. Second. Third."),
            vec!["First.", "Second. Third."]
        );
    }

    #[test]
    fn shaping_matches_desktop_padding_clamp_and_fade() {
        let shaped = shape_tts_chunk(vec![2.0; 400]);
        assert_eq!(shaped.len(), 480 + 400 + 1920);
        assert!(shaped[..480].iter().all(|sample| *sample == 0.0));
        assert_eq!(shaped[480], 1.0);
