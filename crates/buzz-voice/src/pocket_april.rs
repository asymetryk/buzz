...
//!
//! sherpa-onnx 1.13.4 remains Buzz's shared speech runtime, but its Pocket
//! frontend only understands the January vocab/token-score bundle. The April
//! export uses SentencePiece, prepends a learned BOS voice embedding, and
//! describes recurrent state tensors in `bundle.json`. This module implements
//! only that missing frontend/state loop and reuses the ONNX Runtime already
...
use super::{PreparedPrompt, VoiceStyle};
...
const FILE_FLOW_MAIN_FP32: &str = "flow_lm_main.onnx";
const FILE_FLOW_FP32: &str = "flow_lm_flow.onnx";
const FILE_MIMI_DECODER_FP32: &str = "mimi_decoder.onnx";
...
impl AprilPocketTts {
    pub(crate) fn load(dir: &Path) -> Result<Self, String> {
        ...
        Ok(Self {
            mimi_encoder: load_session(dir.join(FILE_MIMI_ENCODER))?,
            text_conditioner: load_session(dir.join(FILE_TEXT_CONDITIONER))?,
            flow_main: load_session(dir.join(FILE_FLOW_MAIN_FP32))?,
            flow: load_session(dir.join(FILE_FLOW_FP32))?,
            mimi_decoder: load_session(dir.join(FILE_MIMI_DECODER_FP32))?,
            bundle,
            tokenizer,
            bos_embedding,
            cached_voice: None,
